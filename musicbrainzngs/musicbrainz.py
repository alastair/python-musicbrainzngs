...
RELATABLE_TYPES = ['area', 'artist', 'label', 'place', 'event', 'recording',
                   'release', 'release-group', 'series', 'url',
                   'work', 'instrument']
...
VALID_BROWSE_INCLUDES = {
    ...
    'recording': ["artist-credits", "isrcs"] + TAG_INCLUDES +
                 RATING_INCLUDES + RELATION_INCLUDES,
    ...
    'release-group': ["artist-credits"] + TAG_INCLUDES + RATING_INCLUDES +
                     RELATION_INCLUDES,
    'url': RELATION_INCLUDES,
    'work': ["aliases", "annotation"] + TAG_INCLUDES + RATING_INCLUDES +
            RELATION_INCLUDES,
}
...
VALID_SEARCH_FIELDS = {
    ...
    'place': [
        'address', 'alias', 'area', 'begin', 'comment', 'end', 'ended',
        'lat', 'long', 'pid', 'place', 'placeaccent', 'type'
    ],
    'recording': [
...
def _check_filter_and_make_params(entity, includes, release_status=None,
                                  release_type=None):
...
class _MusicbrainzHttpRequest(compat.Request):
    ...
    def __init__(self, method, url, data=None):
        ...
        if method not in allowed_m:
            raise ValueError(f"invalid method: {method}" % method)
        self.method = method
...
@_rate_limit
def _mb_request(path, method='GET', auth_required=AuthNo,
    ...
    if _useragent == "":
        raise UsageError('set a proper user-agent with '
                         'set_useragent("application name", '
                         '"application version", '
                         '"contact info (preferably URL or email for your '
                         'application)")')
    ...
    if auth_required == AuthIfset and user:
...
def _do_mb_delete(path):
    ...
    """
    return _mb_request(path, 'DELETE', AuthYes, True)


def _do_mb_put(path):
    """Send a PUT request for the specified object.
    """
    return _mb_request(path, 'PUT', AuthYes, True)
...
def _do_mb_post(path, body):
    ...
    request body.
    """
    return _mb_request(path, 'POST', AuthYes, True, body=body)
...
def get_releases_by_discid(id, includes=[], toc=None, cdstubs=True, media_format=None):
    ...
    *Available includes*: {includes}"""
    params = _check_filter_and_make_params("discid", includes,
                                           release_status=[], release_type=[])
    if toc:
        params["toc"] = toc
    if not cdstubs:
...
def _browse_impl(entity, includes, limit, offset, params,
    ...
    filterp = _check_filter_and_make_params(entity, includes, release_status,
                                            release_type)
    p.update(filterp)
